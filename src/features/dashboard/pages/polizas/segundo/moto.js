...
const SegundoMoto = (props) => {
    ...
    useEffect(() => {
        if( listadoMarcas.length === 0 ) {
            ...
            const options = {
                url: `http://3.136.94.107:4300/api/MarcaMoto`,
                method: 'GET',
                headers: {
    ...
    const handleChangeMarca = (event) => {
        ...
        const source = CancelToken.source();
        const options = {
            url: `http://3.136.94.107:4300/api/YearMoto?Marca=${event.target.value}`,
            method: 'GET',
            headers: {
                'Authorization': `Bearer ${auth.tokenFirebase}`,
                'Content-Type': 'application/json',
    ...
    const handleChangeYear = (event) => {
        ...
        const source = CancelToken.source();
        const options = {
            url: `http://3.136.94.107:4300/api/ModeloMoto?Marca=${marca}&Year=${event.target.value}`,
            method: 'GET',
            headers: {
                'Authorization': `Bearer ${auth.tokenFirebase}`,
                'Content-Type': 'application/json',
    ...
    return <div>
        ...
        <Grid container spacing={3}>
            <Grid item xs={12}>
                <Typography variant="h6">Datos para Póliza Moto</Typography>
            </Grid>
